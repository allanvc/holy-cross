    ...
    if reset and reset_start_date:
        ...
        return [
            dict(type="rect", x0=reset_start_date - pd.Timedelta(days=forming_days), x1=reset_start_date,
                 y0=min_z, y1=max_z, fillcolor="rgba(128, 128, 128, 0.3)", line=dict(width=0)),  # Gray
            dict(type="rect", x0=reset_start_date,
                 x1=reset_start_date + pd.Timedelta(days=decision_days),
                 y0=min_z, y1=max_z, fillcolor="rgba(96, 45, 137, 0.6)", line=dict(width=0)),  # #602d89 with stronger alpha
            dict(type="rect", x0=reset_start_date + pd.Timedelta(days=decision_days),
                 ...
                 y0=min_z, y1=max_z, fillcolor="rgba(34, 139, 34, 0.3)", line=dict(width=0))  # Forest Green
        ]
    elif expand_trading:
...
fig.add_trace(go.Scatter(x=[None], y=[None], mode="markers", marker=dict(size=10, color="rgba(128, 128, 128, 0.3)"), name="Formation"))
fig.add_trace(go.Scatter(x=[None], y=[None], mode="markers", marker=dict(size=10, color="rgba(96, 45, 137, 0.6)"), name="Decision"))
fig.add_trace(go.Scatter(x=[None], y=[None], mode="markers", marker=dict(size=10, color="rgba(34, 139, 34, 0.3)"), name="Trading"))
